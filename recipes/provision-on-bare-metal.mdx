...
For the conceptual overview of what an Agent is, see [Agent (bare-metal)](/en/executors/agent-bare-metal).
...
- **API key** — see [Connect an executor](/en/executors/connect-an-executor) to issue one. You'll need the plaintext value in the next step.
...
See [Agent CLI](/en/reference/agent-cli) for every subcommand.
...
- [Apply executor upgrades](/en/infrastructure/apply-upgrades) — what to do when an upgrade goes sideways.
