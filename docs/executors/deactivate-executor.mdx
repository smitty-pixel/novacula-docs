...
To re-activate later, [rotate the API key](/recipes/rotate-api-keys) and walk the new key out to the host.
...
Both flows go through the [Node lifecycle](/docs/nodes/node-lifecycle).
...
- Writes an `executor.deleted` event to the [Events feed](/docs/notifications/events-feed).
...
Stop and remove the per-node `systemd` units the agent created — see [Agent CLI](/reference/agent-cli) for the unit naming convention.
