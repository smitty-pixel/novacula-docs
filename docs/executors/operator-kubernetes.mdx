...
For the bare-metal Agent, see [Agent (bare-metal)](/en/executors/agent-bare-metal).
...
- **A storage class** with `WaitForFirstConsumer` binding for the node PVCs; the operator probes available classes on startup and reports them in [executor capabilities](/en/executors/executor-capabilities).
...
1. Issue an `operator`-kind API key from the UI: see [Connect an executor](/en/executors/connect-an-executor).
...
Walkthrough: [Provision an executor on Kubernetes](/en/infrastructure/provision-on-kubernetes).
...
- [Provision an executor on Kubernetes](/en/infrastructure/provision-on-kubernetes) — Helm install walkthrough.
- [Environment variables](/en/reference/environment-variables) — operator-side env vars.
- [Apply executor upgrades](/en/infrastructure/apply-upgrades) — manual recovery if a self-update wedges.
