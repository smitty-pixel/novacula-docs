...
For the Operator (Kubernetes) backend, see [Operator (Kubernetes)](/en/executors/operator-kubernetes).
...
- The validator workload sits on dedicated hardware (typical for bare-metal colo).
...
All three are tunable from the agent TOML — see [Configuration reference](/en/reference/configuration).
...
This is what makes self-updates safe: if a new binary fails to start three times in two minutes, the rollback unit takes over without operator intervention. See [Apply executor upgrades](/en/infrastructure/apply-upgrades).
...
The agent ships with a Citty-based CLI for install, lifecycle, status, and manual upgrade. See [Agent CLI](/en/reference/agent-cli) for the full subcommand list.

## See also

- [Provision an executor on bare-metal](/en/infrastructure/provision-on-bare-metal) — install walkthrough.
- [Configuration reference](/en/reference/configuration) — agent TOML schema.
- [Environment variables](/en/reference/environment-variables) — env vars the agent reads.
- [Connect an executor](/en/executors/connect-an-executor) — generate the API key the agent will use.
