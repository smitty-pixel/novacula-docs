...
- A role that can create API keys: **owner** or **admin**. See [Roles and permissions](/docs/platform/roles-and-permissions).
- One of:
...
<Steps>
  <Step title="Open the connect dialog">
    ...
    Give the executor a name. The name is your label — it appears in the executors list, the events feed, and as a target in the [Deploy node wizard](/docs/nodes/deploy-a-node). Names are unique per organization.
  ...
  <Step title="Install on your infrastructure">
    ...
    - **Agent** — see [Provision on bare-metal](/recipes/provision-on-bare-metal) for the full TOML config and `systemd` setup.
    - **Operator** — see [Provision on Kubernetes](/recipes/provision-on-kubernetes) for the Helm install and CRD bootstrap.
    ...
    4. The executor declares its [capabilities](/docs/executors/executor-capabilities) — chains, networks, clients, versions.
  </Step>
...
- An `executor.connected` event is written to the [Events feed](/docs/notifications/events-feed).
