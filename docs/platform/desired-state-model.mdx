...
The control plane is the only writer. Mutations like [Deploy a node](/docs/nodes/deploy-a-node), [Edit configuration](/docs/nodes/edit-configuration), [Upgrade a node](/docs/nodes/upgrade-node), and lifecycle actions all produce a new spec.
...
The executor runs three concurrent loops (see [Agent (bare-metal)](/docs/executors/agent-bare-metal) and [Operator (Kubernetes)](/docs/executors/operator-kubernetes) for backend specifics):
...
If `desiredState` is `running` but `observedStatus` is `stopped` or `error`, the **`node_down`** alert rule fires — see [Alert rules](/docs/notifications/alert-rules).
...
2. **Outbound-only deployment.** The control plane never initiates anything; the executor pulls. See [Architecture](/docs/platform/architecture).
3. **Auditable history.** Every revision is a discrete write; the [Events feed](/docs/notifications/events-feed) records who changed what and when.
