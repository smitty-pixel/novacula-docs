...
Any node can override the org-wide settings independently — useful for canary nodes that should be allowed to run noisy without paging, or critical nodes with stricter thresholds. See [Per-node notification overrides](/docs/notifications/per-node-overrides).
...
2. A `notification.alert.opened` event is written to the [Events feed](/docs/notifications/events-feed).
3. If the org has a configured webhook and `webhookEnabled = true`, a `WebhookDelivery` is fired — see [Webhooks](/docs/notifications/webhooks).
