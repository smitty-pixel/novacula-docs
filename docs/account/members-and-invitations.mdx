...
- API keys remain valid (they're tied to the org, not the user) — see [API keys](/docs/account/api-keys) to rotate any keys the departing member generated.
- The `AuditAction` rows attributed to the user stay in the audit log for compliance.

## Audit

Every invite, role change, and removal is recorded in the [Audit log](/docs/account/organization#audit-log) with the actor, target, and timestamp.
