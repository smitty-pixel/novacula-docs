...
A regular user belongs to exactly one org at a time. (The exception is the platform-level system admin — see [Roles and permissions](/en/platform/roles-and-permissions).)
...
Inviting, removing, and re-roling members live on a separate page. See [Members and invitations](/en/account/members-and-invitations).
