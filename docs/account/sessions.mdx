...
2. Enter the email tied to your account. A reset link is emailed (the email always confirms send, regardless of whether the address is registered, to avoid leaking account existence).
3. Open the link and set a new password.

A successful reset:

- Updates the password hash on your `Account` row.
- Does **not** auto-revoke existing sessions. If you suspect compromise, revoke all sessions from the panel after logging in.
