...
2. Records a `node.lifecycle.requested` event in the [Events feed](/docs/notifications/events-feed) attributed to the user who triggered it.
...
`Restart` is a clean stop + start. Use it after editing config (the [Edit configuration](/docs/nodes/edit-configuration) flow already triggers this implicitly when needed) or to recover a process that's stuck without changing anything else.
...
Lifecycle actions and delete require **owner** or **admin**. Members can only read. See [Roles and permissions](/docs/platform/roles-and-permissions).
