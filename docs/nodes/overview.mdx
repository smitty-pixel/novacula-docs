...
Logs and metrics surface per-process — see [Node logs](/en/nodes/logs) and [Node monitoring](/en/nodes/monitoring) — so you can stream the EL output independently of the CL output, even though they belong to the same node.
...
| Want to | Page |
| --- | --- |
| Create a new node | [Deploy a node](/en/nodes/deploy-a-node) |
| Start, stop, restart, delete | [Node lifecycle](/en/nodes/node-lifecycle) |
| Change resources or config | [Edit node configuration](/en/nodes/edit-configuration) |
| Move to a newer client version | [Upgrade a node](/en/nodes/upgrade-node) |
| Filter / find a node in a long list | [Groups and tags](/en/nodes/groups-and-tags) |
| Watch live process output | [Node logs](/en/nodes/logs) |
| Read sync / peers / health metrics | [Node monitoring](/en/nodes/monitoring) |
...
- **[Agent](/en/executors/agent-bare-metal)** — one `systemd` service per process; data lives under `<data_dir>/nodes/<name>/`.
- **[Operator](/en/executors/operator-kubernetes)** — one `BlockchainNode` CR → one `StatefulSet` (with one container per role) + a `ConfigMap` + a per-storage `PersistentVolumeClaim`.
