...
| **Ink** | `ink-mainnet`, `ink-sepolia` | `el` + `rollup` | `op-geth` · `op-node` | Full | [Ink](/docs/chains/ink) |
...
Archive node type is supported on **Ethereum** and **BSC** only. The remaining chains expose Full only — for archival use cases on those, tune the chain-specific retention / pruning knobs covered on each per-chain page.
