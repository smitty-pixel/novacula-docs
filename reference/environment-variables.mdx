...
The single source of truth for env-var configuration across all three components. For the broader configuration model (TOML for the agent, env-only for the others), see [Configuration reference](/reference/configuration).
...
The agent does not read other env vars at startup — everything else lives in the TOML. See [Configuration reference](/reference/configuration#agent) for the schema.
...
- [Configuration reference](/reference/configuration) — same data organized by component, with TOML / chart context.
