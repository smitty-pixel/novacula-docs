...
This page is the canonical list of every configurable parameter for each Novacula component. For the env vars specifically, see [Environment variables](/en/reference/environment-variables) — that page is the authoritative env-var index.
...
See [Environment variables](/en/reference/environment-variables) for the same list with copy-pastable examples.
...
Per-node config (resource requests, custom config-file overrides, CLI arg overrides, sidecars) is **not** in any of the configs above — it lives in the `NodeSpec` written by the control plane. See [Edit node configuration](/en/nodes/edit-configuration).

## Related

- [Environment variables](/en/reference/environment-variables) — env-var-only index.
- [Agent CLI](/en/reference/agent-cli) — operator-side commands.
- [GraphQL API](/en/reference/graphql-api) — programmatic API surface.
