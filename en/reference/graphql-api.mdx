...
API keys are managed at [API keys](/en/account/api-keys). Use a `purpose=executor` key for an executor; `purpose=personal` for human-driven scripts.
...
The schema is **not** versioned via URL or version field. Backwards-compatible additions (new fields, new queries) ship continuously; backwards-incompatible changes are rare and announced via [release notes](/en/release-notes).
...
- [API keys](/en/account/api-keys) — issue and rotate the credentials this API uses.
- [Roles and permissions](/en/platform/roles-and-permissions) — what each role can do via this API.
