...
| Executor — [Agent](/en/executors/agent-bare-metal) | Your bare-metal hosts | Reconciles specs into `systemd` services |
| Executor — [Operator](/en/executors/operator-kubernetes) | Your Kubernetes clusters | Reconciles specs into `ConfigMap` + `StatefulSet` |
...
If a node is seen at a revision older than the latest, the UI surfaces drift. See [Desired-state model](/en/platform/desired-state-model).
...
See [Execution backends](/en/platform/execution-backends) for what each backend does differently, and [Chains and networks](/en/platform/chains-and-networks) for how chains, networks, roles, and clients are modeled.
